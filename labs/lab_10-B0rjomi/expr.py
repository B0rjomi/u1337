...
class Name(Expr):
    ...
    def eval(self, env):
        ...
        if self.string in env:
            ...
            raise NameError('Имя не существует в этом окружении')
...
class CallExpr(Expr):
    ...
    def eval(self, env):
        ...
        if isinstance(operator, PrimitiveFunction) or isinstance(operator, LambdaFunction):
...
class LambdaFunction(Value):
    ...
    def apply(self, arguments):
        ...
        "*** ТВОЙ КОД ЗДЕСЬ ***"
        some_env = self.parent.copy()
        for params, args in zip(self.parameters, arguments):
            some_env[params] = args
        return self.body.eval(some_env)
