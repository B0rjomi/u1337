...
def scheme_eval(expr, env, _=None): # Третий опциональный аргумент игнорируется
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        check_procedure(operator) # работает и без этого, но необходимо убедиться, что процедура корректна, иначе будут вылезать ошибки
        worked_operands = rest.map(lambda x: scheme_eval(x, env))
        return scheme_apply(operator, worked_operands, env)
        # КОНЕЦ ЗАДАЧИ 5
...
class Frame(object):
    ...
    def make_child_frame(self, formals, vals):
        ...
        if len(vals) != len(formals):
            raise SchemeError("Количество значений аргументов передано некорректно")
        while formals is not nil:
            new_child.define(formals.first, vals.first)
...
class BuiltinProcedure(Procedure):
    ...
    def apply(self, args, env):
        ...
        try:
            ...
            raise SchemeError('передано некорректное число аргументов')
        # КОНЕЦ ЗАДАЧИ 4
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # НАЧАЛО ЗАДАЧИ 6
        look_define = scheme_eval(expressions.second.first, env)
        env.define(target, look_define) # связываем с помощью функции define
        return target
        # КОНЕЦ ЗАДАЧИ 6
    elif isinstance(target, Pair) and scheme_symbolp(target.first):
        # НАЧАЛО ЗАДАЧИ 10
        eval_cut_form = LambdaProcedure(target.second, expressions.second, env)
        env.define(target.first, eval_cut_form)
        return target.first
        # КОНЕЦ ЗАДАЧИ 10
    else:
...
def make_let_frame(bindings, env):
    ...
    while bindings != nil:
        ...
        exp = scheme_eval(binds.second.first, env)
        check_form(binds, 2, 2)
        formals = Pair(binds.first, formals)
        check_formals(formals)
        expressions = Pair(exp, expressions)
        bindings = bindings.second
    return env.make_child_frame(formals, expressions)
...
def do_mu_form(expressions, env):
    ...
    check_formals(formals)
    # НАЧАЛО ЗАДАЧИ 16
    secondary_mu = expressions.second
    return MuProcedure(formals, secondary_mu)
